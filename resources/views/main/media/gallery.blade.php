...
.gallery-title {
    ...
    background: linear-gradient(135deg, #180356, #006795);
...
.gallery-breadcrumb .bc-current { color: black; font-weight: 600; font-size: 17px; }
...
.gallery-empty i { font-size: 3rem; margin-bottom: 16px; color: #334155; }
...
.album-info { padding: 4px 12px;text-align:center;background: #003500}
...
.item-act-btn.move-btn:hover { background: #0369a1; }

.item-act-btn.download-btn { background: #16a34a; color: white; text-decoration: none; }

.item-act-btn.download-btn:hover { background: #15803d; }
...
<div class="g-modal-overlay" id="uploadModal">
    ...
    <div class="upload-modal">
        ...
        <h3><i class="fas fa-cloud-upload-alt" style="color:#22c55e;"></i> Upload Ảnh</h3>
...
const BASE_S3_PATH = 'Thư Viện Ảnh';
...
async function loadGallery(folder) {
    ...
    try {
        ...
        const resp = await fetch("{{ route('media.galleryApi') }}?folder=" + encodeURIComponent(folder));
        ...
        if (data.folders.length > 0) {
            ...
            data.folders.forEach(f => {
                ...
                const fullAlbumPath = currentFolder ? 'Thư Viện Ảnh/' + currentFolder + '/' + f.name : 'Thư Viện Ảnh/' + f.name;
                ...
                <div class="album-card" onclick="loadGallery('${escapeHtml(f.path)}')">
                    ...
                    <div class="album-thumb">
                        ...
                        <div class="album-count-badge">${f.count} ảnh</div>
        ...
        if (data.images.length > 0) {
            ...
            html += '<div class="image-section-title"><i class="fas fa-image"></i> Ảnh</div>';
        ...
        if (data.folders.length === 0 && data.images.length === 0) {
            ...
            if (currentFolder) {
                ...
                backBtn = `<button class="gallery-action-btn" onclick="loadGallery('${escapeJs(parentFolder)}')" style="margin:12px auto 0;"><i class="fas fa-arrow-left"></i> Quay lại</button>`;

            }

            html = `<div class="gallery-empty"><i class="fas fa-images"></i><p>Thư mục này chưa có ảnh nào.</p>${backBtn}</div>`;
...
function renderListView() {
    ...
    galleryImages.forEach((img, idx) => {
        ...
        if (currentPerms.can_rename) {
            ...
            actionsHtml += `<button class="item-act-btn rename-btn" onclick="event.stopPropagation(); openRenameModal('${escapeJs(img.path)}', '${escapeJs(img.name)}', 'file')" title="Đổi tên"><i class="fas fa-edit"></i></button>`;

        }

        if (currentPerms.can_upload) {

            actionsHtml += `<button class="item-act-btn move-btn" onclick="event.stopPropagation(); openMoveModal('${escapeJs(img.path)}', 'file', '${escapeJs(img.name)}')" title="Di chuyển"><i class="fas fa-arrows-alt"></i></button>`;

        }

        if (currentPerms.can_delete) {

            actionsHtml += `<button class="item-act-btn delete-btn" onclick="event.stopPropagation(); openDeleteModal('${escapeJs(img.path)}', 'file', '${escapeJs(img.name)}')" title="Xóa"><i class="fas fa-trash"></i></button>`;
...
function renderItemActions(img, idx) {

    let html = '<div class="item-actions">';

    if (currentPerms.can_rename) {

        html += `<button class="item-act-btn rename-btn" onclick="event.stopPropagation(); openRenameModal('${escapeJs(img.path)}', '${escapeJs(img.name)}', 'file')" title="Đổi tên"><i class="fas fa-edit"></i></button>`;

    }

    if (currentPerms.can_upload) {

        html += `<button class="item-act-btn move-btn" onclick="event.stopPropagation(); openMoveModal('${escapeJs(img.path)}', 'file', '${escapeJs(img.name)}')" title="Di chuyển"><i class="fas fa-arrows-alt"></i></button>`;

    }

    if (currentPerms.can_delete) {

        html += `<button class="item-act-btn delete-btn" onclick="event.stopPropagation(); openDeleteModal('${escapeJs(img.path)}', 'file', '${escapeJs(img.name)}')" title="Xóa"><i class="fas fa-trash"></i></button>`;

    }

    html += `<a class="item-act-btn download-btn" href="${escapeHtml(img.url)}" download="${escapeHtml(img.name)}" onclick="event.stopPropagation()" title="Tải về"><i class="fas fa-download"></i></a>`;
...
function renderFolderTree() {
    ...
    allFolders.forEach(f => {
        ...
        const label = f.path === '' ? 'Thư Viện Ảnh (gốc)' : f.path.split('/').pop();
