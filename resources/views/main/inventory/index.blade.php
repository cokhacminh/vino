...
<div class="inv-left">
    ...
    <div class="inv-section">
        ...
        <div class="inv-body">
            ...
            <thead><tr><th>Ngày</th><th>Nhập/Xuất</th><th>Sản Phẩm</th><th>Số Lượng</th><th>Giá Nhập</th><th style="width:60px"></th></tr></thead>
            <tbody></tbody></table>
        </div>
    </div>
</div>
...
<div class="inv-right">
    <div class="inv-section">
        ...
        <button class="btn-add-inv" onclick="document.getElementById('importModal').classList.add('show')"><i class="fa-solid fa-plus"></i> Thêm</button>
    </div>
</div>
</div>
...
<form id="importForm">
<input type="hidden" name="LoaiPhieu" value="Nhập Kho">
...
function loadData(){
    ...
    $.getJSON('/inventory/data',params,function(data){
        ...
        data.nhapList.forEach(n=>{
            const loai=n.LoaiPhieu==='Nhập Kho'?'<span style="color:#059669;font-weight:600">Nhập Kho</span>':'<span style="color:#dc2626;font-weight:600">Xuất Kho</span>';
        ...
        $('#nhapTable tbody').html(nHtml||'<tr><td colspan="6" style="text-align:center;color:#999;padding:12px">Không có dữ liệu</td></tr>');
        nhapDT=$('#nhapTable').DataTable({order:[[0,'desc']],pageLength:10,language:dtLang,destroy:true,columnDefs:[{orderable:false,targets:[5]}]});
...
function deleteNhap(id){
    if(!confirm('Xóa phiếu nhập/xuất này?'))return;
